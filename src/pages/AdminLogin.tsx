  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (!ALLOWED_ADMINS.includes(trimmedEmail)) {
      toast({
        title: "Access Denied",
        description:
          "This email is not authorized for admin access. Please contact the system administrator.",
        variant: "destructive",
      })
      return
    }
            ...
            {import.meta.env.DEV && (
              <div className="w-full border-t pt-4">
                <Button
                  type="button"
                  variant="outline"
                  className="w-full h-10 border-green-300 hover:bg-green-50 text-green-700"
                  onClick={async () => {
                    const devEmail = "admin@kailashkalamkari.com"
                    const devPassword = "Admin@123456"
                    ...
                    try {
                      const success = await signInAdmin(devEmail, devPassword)
                      if (success) {
                        toast({
                          title: "Login Successful",
                          description: "Welcome back!",
                ...
                >
                  ...
                  {loading ? "Logging in..." : "Quick Admin Login (Dev)"}
                </Button>
              </div>
            )}
