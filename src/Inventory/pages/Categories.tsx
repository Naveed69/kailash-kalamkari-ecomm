...
const Categories = () => {
  ...
  const [itemToDelete, setItemToDelete] = useState<{ type: 'category' | 'subcategory', id: number } | null>(null);
  ...
  const handleSaveCategory = async () => {
    if (!categoryForm.name.trim()) {
      toast({ title: "Validation Error", description: "Category name is required", variant: "destructive", duration: 5000 });
      return;
    }
    ...
    if (error) {
      ...
      toast({ title: "Success", description: `Category ${editingCategory ? "updated" : "created"} successfully`, duration: 5000 });
  ...
  const handleDeleteCategory = async (id: number) => {
    ...
    if (unlinkError) {
      ...
      setLoading(false);
      toast({ title: "Error", description: "Failed to unlink products: " + unlinkError.message, variant: "destructive", duration: 5000 });
      return;
    }
    ...
    if (relatedSubCats.length > 0) {
      const deletePromises = relatedSubCats.map(sc => deleteSubCategory(sc.id));
      ...
      if (subCatError) {
        ...
        setLoading(false);
        toast({ title: "Error", description: "Failed to delete subcategories: " + subCatError.error.message, variant: "destructive", duration: 5000 });
        return;
      }
    }
    ...
    if (error) {
      ...
      toast({ title: "Success", description: "Category and subcategories deleted successfully", duration: 5000 });
      fetchData();
    }
  };
  ...
  const handleSaveSubCategory = async () => {
    if (!subCategoryForm.name.trim() || !subCategoryForm.category_id) {
      toast({ title: "Validation Error", description: "Name and category are required", variant: "destructive", duration: 5000 });
      return;
    }
    ...
    if (error) {
      ...
      toast({ title: "Success", description: `Subcategory ${editingSubCategory ? "updated" : "created"} successfully`, duration: 5000 });
  ...
  const handleDeleteSubCategory = async (id: number) => {
    ...
    if (error) {
      ...
      toast({ title: "Success", description: "Subcategory deleted successfully", duration: 5000 });
      fetchData();
    }
  };
    ...
    <div className="p-8 space-y-8 bg-slate-50/50 min-h-screen font-sans">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 mt-2">Manage your product categories and subcategories</p>
        </div>
        <Button 
      ...
      <div className="grid gap-8 md:grid-cols-2">
          ...
          <div className="p-6 border-b border-slate-100">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-slate-500 mt-1">Main product categories</p>
              </div>
              <Button 
            ...
            <Table>
              <TableHeader className="bg-slate-50/50">
                <TableRow className="hover:bg-slate-50/50 border-slate-100">
                  ...
                  <TableHead className="w-[100px] text-slate-700 font-semibold">Actions</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {categories.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={2} className="text-center py-8 text-slate-400">
                      No categories found. Click "Seed Default Categories" to get started.
                    </TableCell>
                  </TableRow>
                ) : (
                  categories.map((cat) => (
                    <TableRow key={cat.id} className="hover:bg-slate-50/50 border-slate-50">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
          <div className="p-6 border-b border-slate-100">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-lg font-bold text-slate-900">Subcategories</h3>
                <p className="text-sm text-slate-500 mt-1">Product subcategories</p>
              </div>
              <Button 
              ...
              <TableBody>
                {subCategories.length === 0 ? (
                  ...
                  subCategories.map((subCat) => (
                    <TableRow key={subCat.id} className="hover:bg-slate-50/50 border-slate-50">
                      <TableCell className="font-medium text-slate-900">{subCat.name}</TableCell>
                      <TableCell className="text-sm text-slate-500">{getCategoryName(subCat.category_id)}</TableCell>
                      <TableCell>
                        <div className="flex gap-2">
                          <Button 
                            variant="ghost" 
      ...
      <Dialog open={categoryDialog} onOpenChange={setCategoryDialog}>
        <DialogContent className="rounded-2xl shadow-2xl border-0">
          ...
          <div className="space-y-4 py-4">
            <div>
              <Label htmlFor="cat-name" className="text-slate-700 font-medium">Name *</Label>
              <Input
                id="cat-name"
      ...
      <Dialog open={subCategoryDialog} onOpenChange={setSubCategoryDialog}>
        <DialogContent className="rounded-2xl shadow-2xl border-0">
          <DialogHeader>
            <DialogTitle className="text-xl font-bold text-slate-900">{editingSubCategory ? "Edit Subcategory" : "Add New Subcategory"}</DialogTitle>
            <DialogDescription className="text-slate-500">
              {editingSubCategory ? "Update the subcategory details below." : "Create a new subcategory."}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            <div>
              <Label htmlFor="subcat-name" className="text-slate-700 font-medium">Name *</Label>
              <Input
                id="subcat-name"
            ...
            <div>
              <Label htmlFor="subcat-category" className="text-slate-700 font-medium">Category *</Label>
              <select
                id="subcat-category"
